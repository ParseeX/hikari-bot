...
def draw_section(img, card_images, title, start_x, start_y, card_width, card_height, rows, padding):
    ...
    # 绘制标题
    draw.text((start_x + hex_width + 20, title_y), title, fill="black", font=ImageFont.truetype(font_path_sc, 40))
...
async def generate_deck_image(deck_text, id, match, result="", deck_name=""):
    ...
    font = ImageFont.truetype(font_path_sc, 60)
    ...
    font = ImageFont.truetype(font_path_sc, 50)
    ...
    font = ImageFont.truetype(font_path_sc, 80)
    bbox = font.getbbox("神人都市")
    text_height = bbox[3] - bbox[1]
