(function(){
  ...
  Promise.all(tasks).then(responses=>{
    ...
    const entries=body.data.map(r=>({id:r.card_version_id||r.id,card_id:r.card_id,object_type:r.card_object_type||'unknown',name_jp:r.name_jp||'',name_cn:r.name_cn||'',aliases:[r.name_origin,...(r.card_names||[]).map(n=>n.name_value)].filter(n=>typeof n==='string'&&n),number:r.number||'',rarity:r.rarity||''}));
    let product=null;
    if(input.page===1){
      const p=responses[1].result.data;
      if(!p||Number(p.pack_id||p.id)!==input.pack_id||!p.name_cn)throw new Error('invalid product');
      product={id:input.pack_id,name:p.name_cn,name_origin:p.name_origin||'',released_at:p.released_at||null,version_count:p.card_version_count,sample_version_ids:(p.card_versions||[]).map(r=>r.id).filter(Number.isInteger)};
    }
