(function(){
  ...
  require('api/cloud.js').cloudRequest({card_version_id:input.version_id,game_key:'ygo',game_sub_key:'ocg'},'findCardVersion','/api/market/card-versions/'+input.version_id).then(response=>{
    ...
    Object.assign(state,{status:'done',object_type:r.object_type||'unknown',product:{id:Number(p.pack_id),name:p.name_cn,name_origin:p.name_origin||'',released_at:p.released_at||null}});
