...
桥接启动后主动准备一次手机会话，随后才处理查询；服务刚启动时到达的请求仍需等待准备完成。页面打开后检测运行库、业务模块和登录态就绪，立即继续，不再固定等待 8 秒。

服务内每 5 分钟检查一次手机连接、进程身份和小程序 JS 会话。探针只确认业务函数与登录状态存在，不调用集换社价格接口、不导出登录凭据。正常时短暂恢复 JS 循环完成检查，再按前后台状态暂停；会话失效时最多恢复一次，恢复流程保持原来的锁屏处理。用户查询已占用手机时跳过当次检查，共用同一把锁，避免同时操作手机。

检查失败后，下一次分别等待 10、20、30 分钟，之后最多每 30 分钟重试；成功后恢复为每 5 分钟。手机离线时不尝试打开微信。服务停止会取消等待，并等待正在进行的检查结束后再清理手机会话。日志中的 `background session ready recovered=True` 表示本轮提前恢复了会话，`recovered=False` 表示原会话可用。

后台检查可以减少闲置后的冷启动等待，但无法阻止系统在两次检查之间回收进程；查询恰好遇到恢复过程时仍可能等待或返回忙。手机重启后的首次凭据解锁、登录失效和网络中断仍需分别处理。
